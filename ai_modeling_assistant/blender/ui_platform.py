    ...
    def execute(self, context):
        def apply():
            with conversation.store() as db:
                ...
                content = {"project":project_id, "thread":thread_id, "database":str(conversation.database_path()),
                    "memories": [dict(m, versions=db.versions(m['id'])) for m in db.visible(project_id, thread_id)],
                    "summary":db.summary(thread_id)}
                text_view(context, "AI Memory Audit", json.dumps(content, ensure_ascii=False, indent=2))
        return invoke_action(self, apply)
...
class AMA_OT_MemoryReview(bpy.types.Operator):
    ...
    action: EnumProperty(name="Decision", items=[('verify','Confirm / 确认','Use this version, supersede conflicting versions'),
        ('reject','Reject / 拒绝','Keep audit history but do not retrieve'),
        ('restore','Restore as candidate','Review again'), ('forget','Forget / 删除记忆','Erase this memory and all its revisions')])
    ...
    def draw(self, context):
        ...
        labels(self.layout, "Confirming resolves other memories with the same key. Forget removes the memory's versions; source conversation and filesystem backups remain separate.")
    def execute(self, context):
        def apply():
            with conversation.store() as db:
                project_id, thread_id = conversation.identity(context.scene, db)
...
class AMA_OT_MemoryPin(bpy.types.Operator):
    ...
    scope: EnumProperty(items=[('project','Project','Only this project'),('user','Personal','Across local projects'),('session','Session','Expires in one day')])
    def execute(self, context):
        def apply():
            with conversation.store() as db:
                project_id, thread_id = conversation.identity(context.scene, db)
                ...
                item = db.propose(project_id, thread_id, scope=self.scope, kind='constraint', key='pinned-'+str(turn['id']),
                    value=value, sources=[{'turn_id':turn['id'], 'quote':value}], importance=1)
...
class AMA_OT_MemoryBackup(bpy.types.Operator):
    ...
    def execute(self, context):
        def apply():
            import time
            path = conversation.database_path().with_name('memory-backup-'+time.strftime('%Y%m%d-%H%M%S')+'.sqlite3')
            if path.exists():
...
class AMA_OT_ProactiveCheck(bpy.types.Operator):
    ...
    def execute(self, context):
        def apply():
            ...
            conversation.refresh(context.scene)
            suggestions = ["Review unconfirmed/conflicting memories: " + context.scene.ama_props.memory_status]
            suggestions.extend(report['warnings'] + report['errors'])
            text_view(context, "AI Suggested Actions", "Read-only local suggestions; no model call or scene edit.\n\n" + '\n'.join(suggestions))
        return invoke_action(self, apply)
...
class AMA_PT_Conversation(bpy.types.Panel):
    ...
    def draw(self, context):
        ...
        labels(layout.box(), props.chat_preview or "Your conversation is saved locally and survives Blender restarts.", 52)
        ...
        for name in ('semantic_compression','memory_enabled','memory_retrieval','personal_memory','context_budget'):
        ...
        if props.project_id and props.thread_id:
            ...
            for item in sorted(cached.get('memories',[]), key=lambda m: m['status'] not in {'candidate','conflict'})[props.memory_page*8:props.memory_page*8+8]:
                op = layout.operator('ama.memory_review', text=f"{item['status']} · {item['key'][:24]}")
                op.memory_id = item['id']
            layout.prop(props,'thread_page')
            for thread in cached.get('threads',[])[props.thread_page*5:props.thread_page*5+5]:
                if thread['id'] != props.thread_id:
                    op = layout.operator('ama.chat_thread', text=thread['title'][:32] + ' · ' + thread['id'][:5])
                    op.thread_id = thread['id']
            layout.prop(props,'project_page')
            for project in cached.get('projects',[])[props.project_page*5:props.project_page*5+5]:
                if project['id'] != props.project_id:
                    op = layout.operator('ama.memory_projects',text=project['name'][:20]+' · '+project['id'][:8])
...
class AMA_PT_Inputs(bpy.types.Panel):
    ...
    def draw(self, context):
        ...
        labels(layout, 'Draw with native Grease Pencil, then capture the viewport as an image reference. Selected mesh indices become explicit edit anchors.')


CLASSES = [value for value in list(globals().values()) if isinstance(value, type)
           and value.__module__ == __name__ and issubclass(value, (bpy.types.Operator, bpy.types.Panel))]
