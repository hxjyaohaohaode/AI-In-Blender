...
def safe_filename(value):
    ...
    if name.upper().split(".")[0] in {"CON", "PRN", "AUX", "NUL", *(f"COM{i}" for i in range(10)), *(f"LPT{i}" for i in range(10))}:
        name = "asset_" + name
...
def import_artifact(artifact, collection=None):
    ...
    if kind in {"model3d", "world"}:
        if path.suffix == ".glb":
            ...
            result = (bpy.ops.wm.obj_import(filepath=str(path)) if bpy.app.version >= (4, 0, 0)
                      else bpy.ops.import_scene.obj(filepath=str(path)))
        elif path.suffix == ".stl":
            result = (bpy.ops.wm.stl_import(filepath=str(path)) if bpy.app.version >= (4, 1, 0)
