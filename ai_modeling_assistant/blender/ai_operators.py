...
class AMA_OT_SendToAI(Operator):
    ...
    def execute(self, context):
        return invoke_action(self, lambda: runtime.start_single(context.scene, context.scene.ama_props.prompt))
...
class AMA_OT_RefineObject(Operator):
    ...
    def execute(self, context):
        return invoke_action(self, lambda: runtime.start_single(context.scene, context.scene.ama_props.prompt, refine=True))
...
class AMA_OT_ExecuteCode(Operator):
    ...
    def execute(self, context):
        ...
        if not context.scene.ama_props.last_code.strip():
            return {'CANCELLED'}
        return invoke_action(self,lambda:runtime.start_reviewed_code(context.scene,
            context.scene.ama_props.last_code,list(context.selected_objects)))
...
class AMA_OT_ResumeWorkflow(Operator):
    ...
    def execute(self, context):
        return invoke_action(self,lambda:runtime.resume_checkpoint(context.scene,context.scene.ama_props.workflow_report))
...
class AMA_OT_OpenPreview(Operator):
    ...
    def execute(self, context):
        def apply():
            run = runtime.current(context.scene)
            previews = sorted(run.folder.rglob('preview.png'),key=lambda p:p.stat().st_mtime) if run else []
            if not previews:
                raise ValueError('No rendered branch preview is available yet')
            image = bpy.data.images.load(str(previews[-1]),check_existing=True)
            if context.area:
...
CLASSES = [AMA_OT_SendToAI, AMA_OT_RefineObject, AMA_OT_MultiPassGenerate,
    AMA_OT_ExecuteCode, AMA_OT_CodeEditExecute, AMA_OT_CreatePlan, AMA_OT_RunPlan,
    AMA_OT_CancelWorkflow, AMA_OT_RetryWorkflow, AMA_OT_OpenCode, AMA_OT_ProviderAdd,
    AMA_OT_ProviderRemove, AMA_OT_OpenReport, AMA_OT_Demo, AMA_OT_OpenPlan, AMA_OT_ApplyPlan,
    AMA_OT_ApproveQuality, AMA_OT_SelectWorkflow, AMA_OT_ResumeWorkflow, AMA_OT_OpenPreview]
