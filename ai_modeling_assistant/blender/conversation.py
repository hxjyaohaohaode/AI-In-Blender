...
from ..core.context import (build_context, compression_source, validate_summary,
    extractive_summary, apply_proposals, COMPRESSION_SYSTEM, EXTRACTION_SYSTEM)
...
def database_path():
    ...
    return Path(override) if override else Path(bpy.utils.user_resource("DATAFILES", path="AIInBlender", create=True)) / "platform.sqlite3"
...
def identity(scene, db):
    ...
    if not props.project_id:
        props.project_id = next((s.ama_props.project_id for s in bpy.data.scenes if s != scene and s.ama_props.project_id), '') or uuid.uuid4().hex
...
def refresh(scene):
    with store() as db:
        ...
        scene.ama_props.chat_preview = "\n\n".join(f"{t['role']}: {t['content'][:800]}" for t in turns[-4:])
        ...
        scene.ama_props.memory_status = f"{confirmed} confirmed · {candidates} need review · {len(turns)} stored turns"
        _ui_cache[scene.as_pointer()] = {'memories':memories, 'threads':db.threads(project_id), 'projects':db.projects()}
...
class Dialogue:
    def __init__(self, scene, text, *, job_factory=ProcessJob):
        ...
        with store() as db:
            ...
            self.user_turn = db.append(self.thread_id, "user", text, metadata={
                'attachments':items(scene), 'region':json.loads(scene.ama_props.region_json or '[]')})
            if scene.ama_props.semantic_compression:
                self.source = compression_source(db, self.thread_id, self.budget)
        if self.source:
            self.submit("compress", COMPRESSION_SYSTEM, [{"role": "user", "content": json.dumps(self.source, ensure_ascii=False)}])
        else:
            self.respond()

    def submit(self, stage, system, messages):
        self.stage = stage
        self.scene.ama_props.chat_status = {"compress": "Compressing context with source references…",
            "respond": "Thinking with conversation and verified memory…", "extract": "Preparing source-backed memory candidates…"}[stage]
        self.job = self.job_factory({"action": "chat", "config": self.config.payload(), "messages": messages,
            "system_prompt": system, "capability": "chat", "prompt": system, "output_dir": str(self.folder)})

    def respond(self):
        with store() as db:
            context = build_context(db, self.project_id, self.thread_id, budget=self.budget,
                scene=SceneContextGenerator.generate(), use_memory=self.scene.ama_props.memory_retrieval,
                include_user=self.scene.ama_props.personal_memory)
            db.event(self.project_id, "context_built", {k: v for k, v in context.items() if k not in {"system", "messages"}})
        from .inputs import attach_to_messages
        attach_to_messages(self.scene, context["messages"], self.config)
    ...
    def tick(self):
        ...
        if self.stage == "compress":
            with store() as db:
                try:
                    ...
                    body = validate_summary(extract_json(result.get("content", "")), self.source["source_ids"])
                    mode = "semantic"
                except ValueError:
                    body, mode = extractive_summary(self.source), "extractive"
                db.save_summary(self.thread_id, self.source["source_ids"], body, mode=mode)
                db.event(self.project_id, "context_compressed", {"mode": mode, "through_id": max(self.source["source_ids"])})
            self.respond()
            return True
        if result.get("error"):
            raise ValueError(result["error"])
        if self.stage == "respond":
            ...
            with store() as db:
                db.append(self.thread_id, "assistant", content, metadata={"model": self.config.model})
            refresh(self.scene)
            if self.scene.ama_props.memory_enabled:
                with store() as db:
                    source = {"user_turns": [t for t in db.turns(self.thread_id, after=self.user_turn-1) if t['role'] == 'user'],
                              "existing_memories": [{k:m[k] for k in ('key','value','scope','status','revision')}
                ...
                source['user_turns'] = [{k:t[k] for k in ('id','role','content')} for t in source['user_turns']]
                while source['existing_memories'] and tokens(json.dumps(source,ensure_ascii=False))+tokens(EXTRACTION_SYSTEM)>self.budget:
                    source['existing_memories'].pop()
                if tokens(json.dumps(source,ensure_ascii=False))+tokens(EXTRACTION_SYSTEM)>self.budget:
                    self.scene.ama_props.chat_status = 'Reply saved; memory extraction skipped to respect the input budget'
                    return False
                self.submit("extract", EXTRACTION_SYSTEM, [{"role": "user", "content": json.dumps(source, ensure_ascii=False)}])
                return True
        elif self.stage == "extract":
            with store() as db:
                result = apply_proposals(db, self.project_id, self.thread_id, extract_json(result.get("content", "")),
...
def _tick():
    for key, dialogue in list(_jobs.items()):
        try:
            ...
            if dialogue.job:
                dialogue.job.cancel()
            dialogue.scene.ama_props.chat_status = ("Memory maintenance failed; reply preserved: " if dialogue.stage == "extract"
                else "Conversation stopped: ") + str(exc)[:350]
            again = False
        if not again:
            _jobs.pop(key, None)
