...
def draw_providers(layout, prefs):
    ...
    if not prefs.providers:
        labels(layout, "Add providers to route experts to different models. Without profiles, the basic chat settings are used.")
        return
    ...
    if profile.protocol == "bridge":
        labels(layout, "Video/world providers need an Async Jobs Bridge server. See docs/PROVIDERS.md.")
    else:
...
class AMA_PT_MainPanel(Panel):
    ...
    def draw(self, context):
        ...
        for item in runtime.list_runs(context.scene):
            op = layout.operator('ama.select_workflow', text=('▶ ' if item.running else '• ') + (item.workflow.goal[:28] if item.workflow else 'Planning…'))
        ...
        if run and run.pending_quality is not None:
            labels(layout.box(), 'Native checks passed. Inspect the model and available branch preview before visual approval.')
        ...
        if tasks:
            ...
            for task in tasks[:24]:
                ...
                icon = {'succeeded': 'CHECKMARK', 'failed': 'ERROR', 'running': 'TIME',
                        'cancelled': 'CANCEL', 'blocked': 'LOCKED'}.get(state, 'DOT')
            ...
            if not working and any(t.get("status") in {'failed', 'cancelled', 'blocked'} for t in tasks):
...
class AMA_PT_CodeEditorPanel(Panel):
    ...
    def draw(self, context):
        ...
        labels(layout, "Generated Python uses Blender permissions. The code guard is not an OS sandbox.")
...
class AMA_PT_APISettingsPanel(Panel):
    ...
    def draw(self, context):
        ...
        if not prefs or not any(profile.enabled for profile in prefs.providers):
            ...
            for name in ("api_preset", "api_url", "model", "api_key", "key_env", "temperature", "max_tokens", "request_timeout"):
                box.prop(props, name)
        for name in ("artifact_dir", "input_audio", "auto_fix", "max_repairs", "include_history", "max_history_tokens",
                     "basic_vision", "max_parallel", "max_calls", "require_visual_review"):
...
CLASSES = [AMA_UL_Providers, AMA_PT_MainPanel, AMA_PT_GenerationPanel,
           AMA_PT_CodeEditorPanel, AMA_PT_APISettingsPanel]
