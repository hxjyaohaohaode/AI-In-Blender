...
## 3.3：Blender 5.2 LTS 与生产质量检查

以官方 **Blender 5.2.2 LTS** 为当前稳定版基线，保留 3.6 传统安装包和 4.2+ 扩展安装包。版本与协议能力以实际检查结果为准。详细变更和验收步骤见 [3.3 深度检查记录](docs/QUALITY_AUDIT_3_3.md)。

- **最新 API**：兼容 5.2 几何节点的 RNA 参数，统一输入设置接口；节点参数、色带、相机景深、图像像素和区域属性进入冲突检测。
- **质量证据**：拒绝塌缩 UV、断开的材质输出和缺失纹理；检查实际绑定的动作及形态键动画，在指定时间范围内采样求值网格。材质合同增加真实 Cycles CPU 材质预览，并区分几何、材质和动画帧证据。
- **完整场景提交**：新增灯光／相机专家。显式 `allow_scene_settings` 合同允许串行任务提交世界、相机、帧率、色彩管理和渲染设置；普通部件不能修改共享设置。
- **可靠交付**：导出前重检；GLB 通过结构与嵌入资源检查后才替换正式文件；同时保存保留程序化材质、世界和动画的 `asset.blend`。外部纹理／缓存未打包时仍为文件引用。
- **长期记忆**：数据库升级到 v3；失败、修复和成功事件保留历史，防止旧尝试覆盖新结果；过期摘要不能在用户修改／遗忘记忆后写回；超过一万条消息仍从最早未覆盖原文压缩。
- **真实工具修复**：地形细分参数实际生效；散布按求值后三角面的面积采样，贴合斜面并保留源尺寸，不改全局随机状态。

3.2 的工作流机制继续保留，完整状态转移、故障处理和验收边界见 [生产工作流规范](docs/WORKFLOW_LIFECYCLES.md)。
...
启用 Vision 能力的评审专家会收到预算内的真实预览；材料合同增加 Cycles 工作室材质实渲，动画合同增加带帧号的采样图。报告记录实际发送的视图数量，普通文本评审明确不具有视觉证据。预览和有限采样不能证明最终灯光、全部动画帧或审美／语义完全正确。世界模型输出以 3D 场景资产导入，不是通用世界模拟运行时。

## Blender 兼容性

| 版本 | 状态 |
|---|---|
| Windows + Blender 3.6.23 / 5.1.1 / 5.2.2 LTS | 本机验证基线，详见 [验证记录](docs/VALIDATION.md) |
| Linux + 3.6.23 | CI：源码、预设、流程与传统 ZIP 安装 |
| Linux + 4.2.23 / 4.5.14 LTS / 5.1.2 / 5.2.2 LTS | CI：源码、传统 ZIP、真实扩展安装及工作进程 |
| macOS Apple silicon + 5.2.2 LTS | CI：源码、传统 ZIP、扩展及工作进程 |
| 其他版本和平台 | 未列入验收矩阵 |

最低版本为 **Blender 3.6**；不支持 2.x、3.0–3.5，也不保证未来版本无需适配。3.6 与 4.2 作为兼容基线保留，并不表示其仍接受官方维护。表格描述验收范围；每个提交是否通过请查看 [Actions](https://github.com/hxjyaohaohaode/AI-In-Blender/actions)，未运行的矩阵不视为通过证明。
...
- `dist/ai-in-blender-3.3.0-extension.zip`：Blender 4.2+，Preferences → Get Extensions → Install from Disk。
- `dist/ai-in-blender-3.3.0-legacy.zip`：Blender 3.6+，Preferences → Add-ons → Install / Install from Disk。
