...
def triangle_glb():
    ...
    document = {"asset": {"version": "2.0"}, "scene": 0,
        "scenes": [{"nodes": [0]}], "nodes": [{"mesh": 0, "name": "FixtureTriangle"}],
    ...
    raw += b' ' * (-len(raw) % 4)
    return (b'glTF' + struct.pack('<II', 2, 12 + 8 + len(raw) + 8 + len(binary))
            + struct.pack('<I', len(raw)) + b'JSON' + raw
...
class Handler(BaseHTTPRequestHandler):
    ...
    def do_POST(self):
        ...
        if self.path.endswith('/chat/completions'):
            ...
            system = data['messages'][0]['content'] if data['messages'][0]['role'] == 'system' else ''
            prompt = data['messages'][-1]['content']
            if isinstance(prompt,list):
                for part in prompt:
                    if part.get('type')=='image_url':
                        assert base64.b64decode(part['image_url']['url'].split(',',1)[1]).startswith(b'\x89PNG')
            prompt_text = '\n'.join(p.get('text','') for p in prompt if p.get('type')=='text') if isinstance(prompt,list) else prompt
            current_request = '\n'.join(prompt_text.splitlines()[:2])
            if 'persistent Blender production collaborator' in system:
                content = 'Dialogue reply; received ' + str(len(data['messages'])) + ' messages. Keep the blue material and requested dimensions.'
            elif 'Propose useful memories' in system:
                content = '{"memories":[]}'
            elif 'Compress a Blender production conversation' in system:
                source = json.loads(prompt)
                content = json.dumps({'goals':[{'text':'Retain project dimensions and blue style', 'sources':source['source_ids'][:1]}]})
            elif 'production planner' in system:
                content = json.dumps({"tasks": [
                    {"id": "mesh", "expert": "modeler", "prompt": "Create one cube"},
                    {"id": "material", "expert": "material", "prompt": "Give it a blue material", "depends_on": ["mesh"]},
                    {"id": "check", "expert": "inspector", "prompt": "Inspect", "depends_on": ["material"]},
                    {"id": "review", "expert": "reviewer", "prompt": "Review", "depends_on": ["check"]},
                    {"id": "export", "expert": "exporter", "prompt": "Export", "depends_on": ["review"]}]})
            elif 'passed (boolean)' in system:
                content = json.dumps({"passed": 'REJECT_REVIEW' not in current_request, "summary": "Fixture review", "issues": []})
            ...
            return self.send({"choices": [{"message": {"content": content}, "finish_reason": "stop"}],
    ...
    def do_GET(self):
        ...
        if '/jobs/' in self.path:
            ...
            extension = {'world': '.glb', 'model3d': '.glb', 'video': '.mp4', 'image': '.png', 'speech': '.wav'}.get(kind)
            return self.send({"status": "succeeded", "text": "bridge text", "artifacts":
                ([{"url": base + '/asset' + extension, "kind": kind, "extension": extension}] if extension else [])})
        ...
        if self.path == '/asset.mp4':
            ...
            return self.send(fixture.read_bytes() if fixture.exists() else b'fixture-video', content_type='video/mp4')
