...
class TransportTests(unittest.TestCase):
    ...
    def setUpClass(cls):
        cls.server = subprocess.Popen(
            [sys.executable, str(Path(__file__).parent / "fixtures/provider_server.py")],
    ...
    def test_upstream_errors_do_not_leak_keys(self):
        result = self.run_job(
            {
                "action": "chat",
                "config": self.config(model="error").payload(),
                "messages": [{"role": "user", "content": "cube"}],
            }
        )
    ...
    def test_cancellation_kills_worker_and_removes_temporary_files(self):
        job = ProcessJob(
            {
                "action": "chat",
                "config": self.config(model="slow").payload(),
                "messages": [{"role": "user", "content": "cube"}],
            }
        )
    ...
    def test_native_and_bridge_asset_protocols(self):
        for protocol, capability in [
            ...
            with (
                self.subTest(protocol=protocol, capability=capability),
    ...
    def test_transcription_multipart(self):
        with tempfile.TemporaryDirectory() as folder:
            ...
            result = self.run_job(
                {
                    "action": "generate",
                    "config": self.config("transcription").payload(),
            ...
            result = self.run_job(
                {
                    ...
                    "messages": [
                        {
                            "role": "user",
                            "content": [{"type": "text", "text": "Read this sketch"}]
    ...
    def test_bridge_uploads_actual_conditioning_bytes(self):
        ...
        with tempfile.TemporaryDirectory() as folder:
            ...
            result = self.run_job(
                {
                    "action": "generate",
                    "config": self.config("bridge").payload(),
                    "capability": "world",
                    "prompt": "scene",
                    "output_dir": folder,
            ...
            native = self.run_job(
                {
                    "action": "generate",
                    "config": self.config("meshy").payload(),
                    "capability": "model3d",
                    "prompt": "scene",
                    "output_dir": folder,
                    "inputs": [attachment(path)],
    ...
    def test_recovery_queries_known_job_without_another_submission(self):
        with tempfile.TemporaryDirectory() as folder:
            ...
            self.assertFalse(result.get("error"), result)
            path = Path(folder) / "remote-job.json"
            receipt = json.loads(path.read_text())
            self.assertNotIn("sensitive-token", path.read_text())
            receipt["state"] = (
                "pending"  # Simulate interruption after the provider accepted the job.
            )
            path.write_text(json.dumps(receipt), encoding="utf-8")
            with urlopen(self.base + "/stats") as response:
                before = json.load(response)["job_posts"]
            recovered = self.run_job(
                {"action": "recover", "config": config.payload(), "output_dir": folder}
            )
            self.assertFalse(recovered.get("error"), recovered)
            self.assertTrue(recovered["recovered"])
            with urlopen(self.base + "/stats") as response:
                self.assertEqual(json.load(response)["job_posts"], before)
            self.assertEqual(result["artifacts"][0]["sha256"], recovered["artifacts"][0]["sha256"])
