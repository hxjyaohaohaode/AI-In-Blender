...
class BlenderTests(unittest.TestCase):
    ...
    def setUpClass(cls):
        ...
        cls.server = subprocess.Popen([python_executable(), str(ROOT / 'tests/fixtures/provider_server.py')],
    ...
    def test_native_quality_blocks_empty_mesh_and_contract_violation(self):
        ...
        self.assertFalse(evaluate([obj],{'bounds_min':[-.5,-.5,-.5],'bounds_max':[.5,.5,.5]})['passed'])
        self.assertFalse(evaluate([obj],{'anchor_object':obj.name,'anchor_position':[9,0,0]})['passed'])
    ...
    def test_visual_gate_binds_to_current_revision(self):
        ...
        workflow = Workflow.from_plan('cube', {'tasks':[
            {'id':'mesh','expert':'modeler','prompt':'cube'},
            {'id':'export','expert':'exporter','prompt':'export','depends_on':['mesh']}]})
    ...
    def test_independent_parts_run_concurrently_and_assemble(self):
        workflow = Workflow.from_plan('two independent components', {'assembly':{'units':'meters','up':'Z'},'tasks':[
            {'id':'left','expert':'modeler','prompt':'cube','contract':{'part_id':'left'}},
            {'id':'right','expert':'modeler','prompt':'cube','contract':{'part_id':'right'}},
            {'id':'inspect','expert':'inspector','prompt':'both parts','depends_on':['left','right']} ]})
        run = runtime.StudioRun(self.scene,workflow)
        run.start()
        pump(run)
    ...
    def test_precise_region_rejects_out_of_bounds_vertex_edit(self):
        ...
        with self.assertRaisesRegex(ValueError,'outside the selected region'):
            region_preserved([source],[candidate],[{'asset_id':source['ama_asset_id'],'vertices':[0]}])
    ...
    def test_multiple_workflows_keep_separate_assets_and_focus(self):
        workflow = lambda: Workflow.from_plan('separate cube',{'tasks':[{'id':'mesh','expert':'modeler','prompt':'cube'}]})
    ...
    def test_vision_reviewer_receives_real_render_evidence(self):
        ...
        workflow = Workflow.from_plan('cube',{'tasks':[
            {'id':'mesh','expert':'modeler','prompt':'cube'},
            {'id':'review','expert':'reviewer','prompt':'check geometry','depends_on':['mesh']}]})
        run = runtime.StudioRun(self.scene,workflow)
        run.start()
        pump(run)
        self.assertTrue(workflow.complete,self.scene.ama_props.workflow_status)
    ...
    def test_resume_checkpoint_preserves_successful_steps(self):
        workflow = Workflow.from_plan('REJECT_REVIEW',{'tasks':[
            {'id':'mesh','expert':'modeler','prompt':'cube'},
            {'id':'review','expert':'reviewer','prompt':'review','depends_on':['mesh']}]})
        run = runtime.StudioRun(self.scene,workflow)
        run.start()
        pump(run)
    ...
    def test_semantic_compression_uses_model_and_retains_original_turns(self):
        ...
        with conversation.store() as db:
            ...
            for i in range(12):
                db.append(thread,'user' if i%2==0 else 'assistant',f'Detail {i}: '+ 'blue material '*80)
    ...
    def test_scene_commit_waits_for_interactive_edit_mode_to_end(self):
        bpy.ops.mesh.primitive_cube_add()
        obj = bpy.context.object
        run = runtime.start_reviewed_code(self.scene,"import bpy\nbpy.context.object.scale.x=2",[obj])
    ...
    def test_nested_node_changes_invalidate_content_fingerprint(self):
        ...
        before = fingerprint([obj])
        material.node_tree.nodes.get('Principled BSDF').inputs['Base Color'].default_value = (0.17,0.23,0.91,1)
        self.assertNotEqual(before,fingerprint([obj]))
    ...
    def profile(self, protocol):
        ...
        item.options_json = '{"poll_interval": 0.1, "response_format": "wav"}' if protocol == 'speech' else '{"poll_interval":0.1}'
        return item
    ...
    def test_generated_code_failure_cleans_new_objects_and_restores_mesh(self):
        ...
        ok, _ = CodeExecutor.execute("import bpy\nbpy.ops.mesh.primitive_uv_sphere_add()\nraise ValueError('stop')")
        self.assertFalse(ok)
    ...
    def test_video_and_world_bridge_import(self):
        ...
        workflow = Workflow.from_plan('world and video', {'tasks': [
    ...
    def test_repair_applies_corrected_code_exactly_once(self):
        workflow = Workflow.from_plan('FAIL_ONCE', {'tasks': [
            {'id': 'mesh', 'expert': 'modeler', 'prompt': 'FAIL_ONCE'}]})
        run = runtime.StudioRun(self.scene, workflow)
        run.start()
        pump(run)
        self.assertTrue(workflow.complete, self.scene.ama_props.workflow_status)
    ...
    def test_media_models_import_into_blender(self):
        ...
        workflow = Workflow.from_plan('Generate media', {'tasks': [
            {'id': 'model', 'expert': 'model3d', 'prompt': 'triangle'},
            {'id': 'image', 'expert': 'image', 'prompt': 'reference', 'depends_on': ['model']},
    ...
    def test_failed_review_blocks_export_and_retry_keeps_geometry(self):
        workflow = Workflow.from_plan('REJECT_REVIEW', {'tasks': [
            {'id': 'mesh', 'expert': 'modeler', 'prompt': 'cube'},
            {'id': 'review', 'expert': 'reviewer', 'prompt': 'review', 'depends_on': ['mesh']},
            {'id': 'export', 'expert': 'exporter', 'prompt': 'export', 'depends_on': ['review']}]})
        run = runtime.StudioRun(self.scene, workflow)
        run.start()
        pump(run)
        self.assertFalse(workflow.complete)
...
(destination / f'blender-{bpy.app.version_string}.json').write_text(json.dumps({
    ...
    'success': result.wasSuccessful()}, indent=2), encoding='utf-8')
